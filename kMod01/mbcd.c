...
static struct seq_operations mbcd_seq_ops = { .start = mbcd_seq_start,
		.next = mbcd_seq_next, .stop = mbcd_seq_stop, .show = mbcd_seq_show };

//set up sequenze operator
...
static struct file_operations mbcd_proc_ops = { .owner = THIS_MODULE,
		.open = mbcd_proc_open, .read = seq_read, .llseek = seq_lseek,
		.release = seq_release };
...
static void mbcd_create_proc(void) {
	...
	create_proc_read_entry("mbcd_cread", 0 /* default mode */,
			NULL /* parent dir */, mbcd_read_procmem, NULL /* client data */);
