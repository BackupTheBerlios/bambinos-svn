...
public class ObjectFile {
	...
	/**
	 * @param filename The filename of the objectFile (without ending)
	 * @param mode The mode of the file (can be "r" for reading or "rw" for writing)
	 * @author Lacki 
	 */
	public ObjectFile(String filename, String mode) {
		...
		// if the objectFile-object is created in ReadOnly mode, the existing file gets scanned
	...
	/**
	 * Scans the structure of the objectFile and writes the values into the class-properties
	 */
	...
	/**
	 * reads the fixup-table from the object file and returns its elements in an array
	...
	public FixupTableElement[] getFixupTable() {
		...
		// this verifies, that we don't read more than the data from the fixup-table
		// the fixupTableOffset + fixupTableSize tells us how much we need to read to get the whole table
		...
		// the Vector that contains the FixupTableElements is traversed into an array
	...
	/**
	 * reads the offset-table from the object file and returns its elements in an array
	...
	public OffsetTableElement[] getOffsetTable() {
		...
		// this verifies, that we don't read more than the data from the offset-table
		// the offsetTableOffset + offsetTableSize tells us how much we need to read to get the whole table
		...
		// the Vector that contains the OffsetTableElements is traversed into an array
	 ...
	 * this reads full words from the file even if the string doesn't need a full word-space
	 * in that case the additional bytes to a full word are thrown away
	 */
	private String readStringFromFile(char stringDelimiter) {
		...
		// since we are wordaligned we need to read some additional bytes to assure that we have read full words
	 ...
	 * @param the offset where to put to filepointer
	 */
	...
	/**
	 * Reads the current position in the File and returns it as an offset (as long value)
	 ...
	 * @param mode The mode how to open that file (readable (r), writeable (rw))
