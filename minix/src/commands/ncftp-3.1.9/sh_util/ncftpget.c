...
main(int argc, char **argv)
{
	...
	GetoptReset(&opt);
	while ((c = Getopt(&opt, argc, argv, "P:u:j:p:e:d:t:aRTr:vVf:ADzZEFbcCB:W:X:Y:")) > 0) {
		if (c == 'b') {
			batchmode++;
		}
	}
	...
	GetoptReset(&opt);
	while ((c = Getopt(&opt, argc, argv, "P:u:j:p:e:d:t:aRTr:vVf:o:ADzZEFbcCB:W:X:Y:")) > 0) switch(c) {
		case 'P':
